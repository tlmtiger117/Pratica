# 14/03/26
# Tráfego padrão

- Hoje eu pratiquei analisar um pcap do site [https://wiki.wireshark.org/SampleCaptures#sample-captures]
  para melhorar meu raciocínio em relação a cominucação de dispositivos entre si.
  
   - Sinto que fui bem pra minha 1° vez analisando um pcap externo.
   - No 1° dia eu não estava indo tão bem, por isso decidi analisar só um pouco e ir preparar o ambiente
      - arquivos de relatos/esboço(ideias soltas)
      - Flags da comunicação
      - 1° contato com filtros e tráfego externo.

  - Se você quiser usar o mesmo pcap que eu, aqui está o link dele(Você precisa baixar o pcap):
     - https://wiki.wireshark.org/uploads/27707187aeb30df68e70c8fb9d614981/http.cap

  - DESAFIO-PRA-VOCÊ(Se for baixar o pcap):
    
     - Qual host é a impressora(XEROX)?
     - Qual é o server dessa comunicação?
     - Qual os ips dos sites de ADs desse pcap?
     - Qual host é um SO windows?
     - Tem retransmissão? se sim, qual o significado dela?
       
        - [!] Desafio pra dar sentido a sua análise de pcap.

    
        
